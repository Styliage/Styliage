...
def 生成托盘():
    ...
    for i in range(5):
        ...
        if 随机稀有 < 概率_星5精选:
            ...
            随机英雄 = random.random()
            if 随机英雄 < (数量_星4['红'] + 数量_星3['红']) / (总数_星4 + 总数_星3):
                托盘.append(['红', '星43'])
            elif 随机英雄 < (数量_星4['红'] + 数量_星3['红'] + 数量_星4['蓝'] + 数量_星3['蓝']) / (总数_星4 + 总数_星3):
                托盘.append(['蓝', '星43'])
            elif 随机英雄 < (数量_星4['红'] + 数量_星3['红'] + 数量_星4['蓝'] + 数量_星3['蓝'] + 数量_星4['绿'] + 数量_星3['绿']) / (总数_星4 + 总数_星3):
                托盘.append(['绿', '星43'])
            else:
                托盘.append(['白', '星43'])
    return 托盘
...
while 宝珠数量 >= 5:
    ...
    if 出货 or 歪:
        ...
        概率_星5精选 = 0.03 + (int(叠加概率角色数量 / 5) * 0.0025)
        概率_星5 = 0.03 + (int(叠加概率角色数量 / 5) * 0.0025)
        概率_星4 = 0.58 - (int(叠加概率角色数量 / 5) * 0.005 / 0.94 * 0.58)
        概率_星3 = 1 - 概率_星5精选 - 概率_星5 - 概率_星4

    # print('单次抽卡数量：' + str(单次抽卡数量))
    # print('总抽卡数量：' + str(总抽卡数量))
    # print('剩余宝珠数量：' + str(宝珠数量))
    # print('星5精选概率：' + str(概率_星5精选 * 100) + '%')
    # a = input()
